...
(() => {
  ...
  const state = {
    mode: "all",                 // 'all' | 'fav' | 'days'
    filters: { query: "", domain: "", source_app: "", day: "" },
  ...
  function renderActiveFilters() {
    ...
    if (cl) cl.addEventListener("click", (e) => { e.preventDefault(); state.filters = { query: state.filters.query, domain: "", source_app: "", day: "" }; $("#search").value = state.filters.query; refresh(); loadSidebar(); });
  }

  /* ---------------------------------------------------------------- sidebar */
  async function loadSidebar() {
    if (!api) return;
    const [src, days] = await Promise.all([api.sources(), api.days()]);
  ...
  /* ---------------------------------------------------------------- actions */
  async function paste(id, fmt = "") { if (api) await api.paste_clip(id, fmt); }
  ...
  function openContextMenu(e, c) {
    const ctx = $("#ctx");
    const fmtItems = FORMATS.map(([f, label], k) =>
      `<div class="item" data-paste="${f}">${esc(label)}</div>`).join("");
    ctx.innerHTML = `
      <div class="sub-label">Paste as</div>
      ${fmtItems}
      <div class="sep"></div>
    ...
    ctx.onclick = async (ev) => {
      ...
      if (t.dataset.paste != null) await paste(c.id, t.dataset.paste);
